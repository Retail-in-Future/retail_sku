...
docker run --name retail_sku_mysql -e MYSQL_ROOT_PASSWORD=futureretail -e MYSQL_ROOT_HOST='%' -e MYSQL_DATABASE=retail -d mysql/mysql-server:latest
...
&& sed -i -e "s/futureretail/$MYSQL_ENV_MYSQL_ROOT_PASSWORD/g" src/main/resources/application.yml \
&& ./gradlew build --info \
\
